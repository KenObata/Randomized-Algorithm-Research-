...
Based on the randomized candidate method, Bentert and Skowron, denote random variable $X_c$ as the score that candidate c receives from the algorithm. To analyze the algorithm let us denote $C_{higher}$ as a set of candidates having higher E[$X_c$] than that of the manipulator’s target candidate $A$. Similarly, denote $C_{lower}$  as a set of candidates having lower E[$X_c$] than that of the manipulator’s target candidate $A$.%or $A$ itself
